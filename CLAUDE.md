...
- After modifying code, run `graphify update .` to keep the graph current (AST-only, no API cost). Outside the `graphify` branch this is for local queries only: discard it with `git restore graphify-out` before committing.

## Reglas del equipo (obligatorias)

Ver [CONTRIBUTING.md](CONTRIBUTING.md) como fuente de la verdad.
...
- No escribas rutas absolutas (`C:\Users\...`) en archivos versionados (`.claude/settings.json`, `graphify-out/.graphify_root`, etc.).
